...
|------------|----------|----------------|---------------------|---------------|
| **EAP-TLS** | RFC 5216 | Highest | Client + Server | ✅ **Required** — Only acceptable method |
...
> **Policy (Effective 2026):** EAP-TLS is the **only** permitted EAP method for City of New Orleans networks. All other EAP methods are forbidden due to insufficient security guarantees. Devices that cannot support EAP-TLS must use MAC Authentication Bypass (MAB) with appropriate network segmentation and monitoring.
...
> **Policy:** City of New Orleans requires AAL2 minimum (EAP-TLS with device certificates). AAL3 (TPM-backed certificates) is recommended for high-security environments.
...
|---|-------------|----------|------|------|
| 1 | EAP-TLS support (RFC 5216) on authenticator and authentication server | **Yes** | ☐ | ☐ |
