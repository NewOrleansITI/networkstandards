...
This document establishes the City of New Orleans standards for implementing IEEE 802.1X port-based network access control across wired and wireless infrastructure. 802.1X provides authentication before granting network access, ensuring only authorized devices and users connect to municipal networks.
...
| NIST SP 800-63B | Digital Identity Guidelines | June 2017 | Authentication assurance levels |
...
| EAP Method | Standard | Security Level | Certificate Required | Recommended Use |
|------------|----------|----------------|---------------------|-----------------|
| **EAP-TLS** | RFC 5216 | Highest | Client + Server | Primary method for managed devices |
| EAP-TEAP | RFC 7170 | High | Server (client optional) | Devices without certificates |
| PEAP (MSCHAPv2) | Microsoft | Medium | Server only | Legacy compatibility only |
| EAP-TTLS | RFC 5281 | Medium | Server only | Legacy compatibility only |
| EAP-MD5 | RFC 3748 | Low | None | Not recommended |
...
flowchart TD
    ...
    Q1 -->|No| Q3{Device supports<br/>802.1X at all?}

    Q2 -->|Yes| EAPTLS["✅ EAP-TLS<br/>(Recommended)"]
    Q2 -->|No| TEAP["EAP-TEAP<br/>(Server cert + password)"]

    Q3 -->|Yes| PEAP["PEAP/EAP-TTLS<br/>(Legacy method)"]
    Q3 -->|No| MAB["MAC Authentication Bypass<br/>(Lowest security)"]
...
### EAP-TLS Requirements (Recommended)

EAP-TLS provides the highest security through mutual certificate authentication:
...
| TLS version | TLS 1.2 minimum, TLS 1.3 preferred | Protocol security |
...
| Assurance Level | Authentication Method | 802.1X Equivalent |
|-----------------|----------------------|-------------------|
| AAL1 | Single-factor | Username/password (not recommended) |
| AAL2 | Multi-factor | EAP-TLS with device certificate |
| AAL3 | Hardware crypto | EAP-TLS with TPM-backed certificate |
...
4. IETF RFC 5216, "The EAP-TLS Authentication Protocol," IETF, March 2008.
...
9. NIST SP 800-63B, "Digital Identity Guidelines: Authentication and Lifecycle Management," NIST, June 2017.
10. NIST SP 800-127, "Guide to Securing WiMAX Wireless Communications," NIST, September 2010.
